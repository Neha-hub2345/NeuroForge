...
import { CheckCircle2, XCircle, GitCommitHorizontal, Clock, Loader2, CircleDashed, X, Box, TestTube2, Activity, Settings, GitBranch, Calendar, User, Globe, Cpu, HeartPulse, Tag, FolderKanban, Pencil } from 'lucide-react'
                  ...
                  {builds.map((b) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="pipeline-commit" style={{ verticalAlign: 'middle' }}>
                          <span className="pipeline-commit-inner">
                            <GitCommitHorizontal size={13} /> {b.commitHash ? b.commitHash.substring(0, 7) : '—'}
                          </span>
                        </td>
                        <td style={{ verticalAlign: 'middle' }}>{ENV_LABEL[b.environment] || b.environment || '—'}</td>
                        <td style={{ verticalAlign: 'middle' }}>
                          {b.finishedAt ? (
                        ...
                        <td style={{ verticalAlign: 'middle' }}>{formatIST(b.startedAt)}</td>
                      </tr>
                    )
                  })}
                </tbody>
        ...
        <div style={{
          ...
          <div className="panel bd-modal" style={{
            ...
            <div className="bd-header">
              ...
              <button onClick={() => setSelectedBuildId(null)} className="bd-close">
                <X size={18} />
              </button>
            </div>
                ...
                <>
                  ...
                  <div className="bd-commit-card">
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div className="bd-overview-label" style={{ marginBottom: '4px' }}>Commit Message</div>
                            ...
                            <div className="bd-gauge-wrap">
                              <svg viewBox="0 0 120 120" width="120" height="120">
                                ...
                                <circle
                                  ...
                                  strokeDashoffset={2 * Math.PI * 52 * (1 - Math.min(buildDetails.tests.coveragePercent, 100) / 100)}
                                  transform="rotate(-90 60 60)"
                                />
                              </svg>
                              <div className="bd-gauge-center">
                                <div className="bd-gauge-value">{buildDetails.tests.coveragePercent.toFixed(1)}%</div>
                                <div className="bd-gauge-label">Overall Coverage</div>
                              </div>
                            </div>
                            <div style={{ display: 'flex', gap: '12px', textAlign: 'center', marginTop: '16px' }}>
                              <div style={{ flex: 1, padding: '10px', background: 'var(--success-soft)', color: 'var(--success)', borderRadius: '8px' }}>
                              ...
                              <div style={{ flex: 1, padding: '10px', background: 'var(--danger-soft)', color: 'var(--danger)', borderRadius: '8px' }}>
                              ...
                              <div style={{ flex: 1, padding: '10px', background: 'var(--surface-2)', color: 'var(--ink-soft)', borderRadius: '8px' }}>
                        ...
                        <div>
                          ...
                          <div className="panel bd-deploy-panel">
                            <div className="bd-deploy-row">
                              ...
                              <span className="badge badge-in_progress">{buildDetails.deployment.environment}</span>
                            </div>
                            <div className="bd-deploy-row">
                              <span className="bd-deploy-label"><Tag size={14} /> Image Tag</span>
                              <span className="bd-deploy-mono">{buildDetails.deployment.imageTag}</span>
                            </div>
                            <div className="bd-deploy-row">
                              <span className="bd-deploy-label"><HeartPulse size={14} /> Container Health</span>
                              <span className="badge badge-success">{buildDetails.deployment.podsRunning} / {buildDetails.deployment.podsTotal} Running</span>
                            </div>
                            <div className="bd-deploy-row bd-deploy-row-stacked">
                              ...
                              <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', minWidth: '160px' }}>
                                ...
                                <div className="bd-resource-track">
                                  <div className="bd-resource-fill" style={{ width: `${Math.min(buildDetails.deployment.cpuPercent, 100)}%` }} />
                                </div>
                                <div className="bd-resource-line">
                                  <span>Mem</span><span>{buildDetails.deployment.memoryPercent}%</span>
                                </div>
                                <div className="bd-resource-track">
                                  <div className="bd-resource-fill" style={{ width: `${Math.min(buildDetails.deployment.memoryPercent, 100)}%` }} />
                                </div>
                              </div>
                            </div>
                          </div>
